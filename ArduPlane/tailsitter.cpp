...
bool Tailsitter::transition_fw_complete(void)
{
    ...
    // Get the difference in the pitch initial value to the current pitch initial value to check for discontinuities
    float fw_initial_pitch_diff = (transition->prev_fw_initial_pitch - transition->fw_transition_initial_pitch)*0.01f;

    // Guard for large initial pitch value, if found, we need to reset the initial pitch
    if(abs(fw_initial_pitch_diff) >= 10.0f){
        gcs().send_text(MAV_SEVERITY_WARNING, "Transition FW angle discontinuity, reseting the initial pitch");
        transition->restart();
