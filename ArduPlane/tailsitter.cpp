...
void Tailsitter::output(void)
{
    ...
    int32_t pitch_error_cd_aa = 0, pitch_error_cd = 0;
    ...
    if (vectored_hover_gain > 0) {
        ...
        pitch_error_cd = (des_pitch_cd - quadplane.ahrs_view->pitch_sensor) * 0.5;

        Vector3f att_err_axis_ang;
        quadplane.attitude_control->get_attitude_error_quat().to_axis_angle(att_err_axis_ang);
        pitch_error_cd_aa = degrees(att_err_axis_ang.y) * 0.5 * 100;
        
        float extra_pitch = constrain_float(pitch_error_cd_aa, -SERVO_MAX, SERVO_MAX) / SERVO_MAX;
    ...
    AP::logger().WriteStreaming("PHID", "TimeUS,DesL,DesR,AhrsPitch,WVGain,WVGainS,PitEAA,PitEOld",
        ...
        (float)pitch_error_cd_aa/100, (float)pitch_error_cd/100);
