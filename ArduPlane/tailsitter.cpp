...
void Tailsitter::output(void)
{
    ...
    tilt_left = 0.0f;
    tilt_right = 0.0f;
    float pitch_cd = 0.0f, weathervane_gain = 0.0f, gain_slope = 0.0f, roll_cd = 0.0f;
    int32_t pitch_error_cd_att = 0, pitch_error_cd = 0;
    ...
    if (vectored_hover_gain > 0) {
        ...
        pitch_error_cd_att = (des_pitch_cd - quadplane.ahrs_view->pitch_sensor) * 0.5;

        // Use attitude controller error rotation vector y component for pitch error
        pitch_error_cd = degrees(quadplane.attitude_control->get_att_error_rot_vec_rad().y) * 0.5 * 100;
        ...
        // Check for ATT roll also, if within 40 degrees, only then allow for wvane gain
        roll_cd = quadplane.ahrs_view->roll_sensor;
        if (fabsf(roll_cd) >= 40*100.0f) {
            weathervane_gain = 0.0;
            quadplane.weathervane->reset();
        }
    ...
    AP::logger().WriteStreaming("PHID", "TimeUS,DesL,DesR,AhrsPitch,WVGain,WVGainS,PitEAtt,PitE,AttRll",
            ...
            (float)pitch_error_cd_att/100, (float)pitch_error_cd/100,roll_cd/100);
