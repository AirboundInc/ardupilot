--[[
    ...
    - Added EC20 support (banner EC20C): same AT command set as EC200,
    ...
    - Added CEREG smart reconnect: AT+CEREG=1 enabled in step_CONFIG so
      the modem sends +CEREG: 1 URC on the AT channel when it re-registers
      after a band switch. On timeout, if +CEREG: 1 was seen recently the
      script skips the full ATI restart and jumps straight to CIPOPEN,
      reducing cross-band handoff recovery from 45-70s to ~5s.
--]]
...
--[[
    configuration step.
    AT+CEREG=1 enables unsolicited network registration messages on the
    CMUX AT channel. The modem will send +CEREG: 1 when it re-registers
    after a band switch, allowing the script to skip the full ATI restart
    and jump straight to CIPOPEN for faster reconnection.
--]]
...
--[[
    ...
    Now also watches for +CEREG URCs to track network registration state.
    +CEREG: 1 = registered home network (fast reconnect path available)
    +CEREG: 5 = registered roaming     (fast reconnect path available)
    +CEREG: 0 = not registered         (clear state, need full reset)
    +CEREG: 2 = searching              (clear state, need full reset)
--]]
local function handle_AT_reply(s)
    ...
    -- watch for CEREG unsolicited registration status changes
    local cereg = s:match('%+CEREG:%s*(%d+)')
    if cereg then
...
local function step_CONNECTED()
    ...
    elseif LTE_TIMEOUT:get() > 0 and now_ms - last_data_ms > uint32_t(LTE_TIMEOUT:get() * 1000) then
        --[[
            CEREG smart reconnect decision:

            If the modem sent +CEREG: 1 on the AT channel recently, it means
            it re-registered on the network after a band switch on its own.
            The radio is back up — we only need to reopen the UDP socket.
            Skip the full ATI/CMUX/CREG restart and jump straight to CIPOPEN.

            If no CEREG was seen, the modem fully lost registration or went
            silent for an unknown reason — do the full reset to be safe.
        ...
        if cereg_registered_ms ~= uint32_t(0) and cereg_age < uint32_t(30000) then
            ...
            gcs:send_text(MAV_SEVERITY.ERROR, 'LTE_modem: timeout — full reset')
            reset_to_ATI()
        end
        return
    end
