...
void AC_PosControl::update_z_controller()
{
    ...
    if (_vibe_comp_enabled) {
        ...
        thr_out = _pid_accel_z.update_all(_accel_target.z, z_accel_meas, _dt,
                                          (_motors.limit.throttle_lower || _motors.limit.throttle_upper)) * 0.001f;
        thr_out += _pid_accel_z.get_ff() * 0.001f;
    }
    ...
    // ----------------------------
    // Add acceleration-based thrust control logic (from _accelerationControl)
    // ----------------------------

    // Create a 3D acceleration setpoint vector (assuming lateral components from pos/vel controllers)
    Vector3f acc_sp = Vector3f(_accel_target.x, _accel_target.y, _accel_target.z + GRAVITY_MSS);

    // Compute body Z direction
    Vector3f body_z = Vector3f(-acc_sp.x, -acc_sp.y, GRAVITY_MSS).normalized();

    // Limit tilt (you can reuse existing attitude limit)
    // ControlMath::limitTilt(body_z, Vector3f(0, 0, 1), radians(35.0f)); // example tilt limit

    // Scale thrust relative to hover thrust
    float collective_thrust = acc_sp.z * (_motors.get_throttle_hover() / GRAVITY_MSS) - _motors.get_throttle_hover();

    // Project thrust to vertical axis
    collective_thrust /= (Vector3f(0, 0, 1).dot(body_z));
    collective_thrust = constrain_float(collective_thrust, 0.0f, 1.0f);

    // Final thrust vector
    Vector3f thr_sp = body_z * collective_thrust;

    // ----------------------------
    // Actuator commands
    // ----------------------------
    thr_out += thr_sp.z;
    // Option 1: use scalar throttle as before (keeps full backward compatibility)
    _attitude_control.set_throttle_out(thr_out, true, POSCONTROL_THROTTLE_CUTOFF_FREQ_HZ);

    // Option 2 (optional future extension): feed vector thrust to attitude control if supported
    // _attitude_control.set_thrust_vector(thr_sp);
