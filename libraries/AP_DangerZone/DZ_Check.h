...
//
// Every check carries an optional `duration_ms` debounce: the raw predicate
// must hold continuously for that long before the check reports satisfied
// (0 = instantaneous). `Duration` is therefore just `Threshold` with a
// non-zero duration; it is kept as a distinct name/tag for readability and
// logging.
//
...
struct DZ_ThresholdParams { float thresh; DZ_Cmp cmp; uint32_t duration_ms; };
...
struct DZ_CheckState {
    ...
    void reset() { buf.reset(); raw_since_ms = 0; raw_active = false; }
};
...
struct DZ_Check {
    ...
    static constexpr DZ_Check Threshold(DZ_Getter g, DZ_ThresholdParams p) {
        return { DZ_CheckType::THRESHOLD, g, p.cmp, DZ_Stat::MEAN, p.thresh, 0, 0, p.duration_ms };
    }
